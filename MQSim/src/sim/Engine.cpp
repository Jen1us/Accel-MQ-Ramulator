...
namespace MQSimEngine
{
	...
	void Engine::Initialize_simulation()
	{
		if (started) {
			return;
		}
	...
	void Engine::Run_until(sim_time_type until_time)
	{
		Initialize_simulation();

		if (until_time <= _sim_time) {
			return;
		}
		...
		// Advance the simulator's notion of time even if there were no events.
		if (!stop) {
			_sim_time = until_time;
		}
	}
	...
	{
		return integrated_execution_mode;
	}
}
