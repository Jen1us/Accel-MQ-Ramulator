...
OUT_DIR="${OUT_DIR:-$ROOT/hw_run/traces_jenius/dram_access_driver}"
...
HBF_CONFIG="${HBF_CONFIG:-$ROOT/workloads/dram_access_driver/hbf.config}"
...
echo "[3/6] Build DRAM access CUDA app"
mkdir -p "$OUT_DIR"
nvcc -O2 -arch="$CUDA_ARCH" \
  "$ROOT/workloads/dram_access_driver/dram_access_driver.cu" \
  -o "$OUT_DIR/dram_access_driver"
...
CUDA_INJECTION64_PATH="$ROOT/util/tracer_nvbit/tracer_tool/tracer_tool.so" \
  "$OUT_DIR/dram_access_driver" | tee "$OUT_DIR/app.log"
...
echo "  $OUT_DIR/hbf_requests.trace"
